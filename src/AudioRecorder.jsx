...
function AudioRecorder({ isNoiseSuppressionEnabled, setIsNoiseSuppressionEnabled }) {
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      if (isNoiseSuppressionEnabled) {
        ...
        sourceRef.current.connect(destinationStream);
  ...
  const toggleNoiseSuppression = () => {
    ...
    if (isRecording && sourceRef.current && noiseSuppressorNodeRef.current) {
      sourceRef.current.disconnect();
      ...
      if (!isNoiseSuppressionEnabled) {
        sourceRef.current.connect(noiseSuppressorNodeRef.current);
        noiseSuppressorNodeRef.current.connect(destinationStream);
      } else {
        sourceRef.current.connect(destinationStream);
      }
...
AudioRecorder.propTypes = {
  isNoiseSuppressionEnabled: PropTypes.bool.isRequired,
  setIsNoiseSuppressionEnabled: PropTypes.func.isRequired,
};
